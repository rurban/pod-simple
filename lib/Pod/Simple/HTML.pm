...
sub __adjust_html_h_levels ($self) {
...
sub do_section ($self, $name, $token) {
  return $name;
}
...
sub do_url_link ($self, $token) { return $token->attr('to') }

sub do_man_link ($self, $link) {
  my $to = $link->attr('to');
...
sub do_pod_link ($self, $link) {
  ...
  if(defined $to and length $to) {
    ...
    if(defined $there and length $there) {
      ...
      $there = $self->resolve_pod_page_link($to); # $section is yet unused
...
sub section_escape ($self, str $section) {
  return $self->section_url_escape(
    $self->section_name_tidy($section)
  );
}

sub section_name_tidy ($self, str $section) {
  $section =~ s/^\s+//;
  $section =~ s/\s+$//;
  $section =~ tr/ /_/;
  if ($] ge 5.006) {
...
sub section_url_escape  ($self, @args) { $self->general_url_escape(@args) }
...
sub resolve_pod_page_link ($self, $it?) {
  # resolve_pod_page_link must return a properly escaped URL
  return $self->batch_mode()
   ? $self->resolve_pod_page_link_batch_mode($it)
   : $self->resolve_pod_page_link_singleton_mode($it)
  ;
}
...
sub resolve_pod_page_link_batch_mode ($self, str $to) {
...
sub resolve_pod_link_by_table ($self, $to?, $section?) {
  # A crazy hack to allow specifying custom L<foo> => URL mappings

  return unless $self->{'podhtml_LOT'};  # An optimizy shortcut
...
sub linearize_tokens ($self, @tokens) {
  ...
  while($t = shift @tokens) {
    if(!ref $t or !UNIVERSAL::can($t, 'is_text')) {
      ...
      my $x_open = 1;
      while($x_open) {
        next if( ($t = shift @tokens)->is_text );
...
sub unicode_escape_url ($self, str $string) {
